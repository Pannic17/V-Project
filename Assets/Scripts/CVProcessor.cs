...
public static class CVProcessor
{
    ...
    /// <summary>
    /// Get a list of detected faces as Vector3 Position
    /// NOTE that the depth/z axis is defined by the size of the face #MIRROR
    /// smaller z means far away from the screen, in this case the planet relate to the person should also be far away
    /// </summary>
    /// <param name="faceBox"> the detected boxes of faces </param>
    /// <returns> the center of faces </returns>
    public static List<Vector3> GetFacesCenter(CVRect[] faceBox)
    {
        List<Vector3> facesCenter = new List<Vector3>();
    ...
    {
        Mat pixels = Mat2Vector(input, cluster, mode);
        Mat labels = new Mat();
        ...
        Mat result = GetCenterResult(center, cluster, mode);
    ...
    /// <returns></returns>
    /// This is Neural Network Function
    ...
    /// <summary>
    /// Normalize the pixel value to -1~1 and initialize the tensor for prediction
    /// </summary>
